...
export interface IViewerSocketEvents extends events.EventEmitter {
...
const rfbProtocolVersionFormat = "RFB %03d.%03d\n";
const rfbProtocolKeepAlive = "REP %03d.%03d\n";
const rfbProtocolMajorVersion = 0;
const rfbProtocolMinorVersion = 0;

export class ViewerSocket extends events.EventEmitter implements IViewerSocketEvents  {
  ...
  private firstWrite() {
    const rfbProtocol = printf(rfbProtocolVersionFormat, rfbProtocolMajorVersion, rfbProtocolMinorVersion);
    this._socket.write(Buffer.from(rfbProtocol));
  }
